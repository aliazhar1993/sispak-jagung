<div class="col col-md-12 col-sm-12">
  ...
  <h3 class="menu-second"><a href="#" onclick="window.open('../cetak/?menu=gejala','page','toolbar=0,scrollbars=1,location=0,menubar=0,resizable=0,width=750,height=600,left=50,top=50,titlebar=yes')"><i class="fa fa-print"></i> Cetak</a></h3>
    ...
    if (isset($_GET['status'])) {
        switch ($_GET['status']) {
            case 'ubah-berhasil': echo '<div class="notif-box">Ubah data gejala berhasil.</div>'; break;
            case 'hapus-berhasil': echo '<div class="notif-box">Hapus data gejala berhasil.</div>'; break;
        }
    }
    ?>
    ...
    <?php
    $no=1; $result=$koneksi->query("SELECT * FROM gejala order by idgej desc");
    while($row=mysqli_fetch_array($result,MYSQLI_BOTH)) {
		if ($row['gambar_gej']=="") {$gambar="";} else {$gambar='<img src="../images/gejala/'.$row['gambar_gej'].'" class="img-pen-list"/>';}
